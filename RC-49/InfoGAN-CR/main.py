...
from utils import *
from models import *
from trainer import train_infogan, sample_infogan_given_labels
...
#-------------------------------
# output folders
path_to_output = os.path.join(wd, "output/output_arch_{}_dimz_{}_dimc_{}_lambda_{}_lrg_{}_lrd_{}".format(args.GAN_arch, args.dim_z, args.dim_c, args.lambda_info, args.lr_g, args.lr_d))
...
Filename_GAN = save_models_folder + '/ckpt_{}_dimz_{}_dimc_{}_niters_{}_seed_{}.pth'.format(args.GAN_arch, args.dim_z, args.dim_c, args.niters_gan, args.seed)
print(Filename_GAN)

if not os.path.isfile(Filename_GAN):

        if args.GAN_arch=="DCGAN":
            netG = DCGAN_Generator(dim_z=args.dim_z, dim_c=args.dim_c).cuda()
            netD = DCGAN_Discriminator().cuda()
        else:
            netG = SNGAN_Generator(dim_z=args.dim_z, dim_c=args.dim_c).cuda()
            netD = SNGAN_Discriminator().cuda()
        netDH = DHead().cuda()
        netQH = QHead(dim_c=args.dim_c).cuda()
        netCR = DCGAN_Discriminator_CR(dim_c=args.dim_c).cuda()
        netG = nn.DataParallel(netG)
        netD = nn.DataParallel(netD)
        netDH = nn.DataParallel(netDH)
        netQH = nn.DataParallel(netQH)
        netCR = nn.DataParallel(netCR)

        # Start training
        netG, netD, netDH, netQH, netCR = train_infogan(images_train, args.cr_gap, netG, netD, netDH, netQH, netCR, save_images_folder=save_images_folder, save_models_folder = save_models_folder)
...
def fn_sampleGAN_given_labels(labels, batch_size):
    ...
    fake_images, fake_labels = sample_infogan_given_labels(netG, given_labels=labels, label_loc=args.label_loc, reverse_label=args.reverse_label, batch_size = batch_size)
    fake_labels = fake_labels / args.max_label
    return fake_images, fake_labels

stop = timeit.default_timer()
print("InfoGAN training finished; Time elapses: {}s".format(stop - start))
...
if args.comp_FID:
    ...
    if args.dump_fake_for_NIQE:
        ...
        dump_fake_images_folder = save_images_folder + '/fake_images_for_NIQE_nfake_{}_label_loc_{}'.format(len(fake_images), args.label_loc)
    ...
    eval_results_logging_fullpath = os.path.join(path_to_output, 'eval_results_label_loc_{}.txt'.format(args.label_loc))
...
if args.visualize_fake_images:
    ...
    filename_fake_images = os.path.join(save_images_folder, 'fake_images_label_loc_{}_grid_{}x{}.png').format(args.label_loc, n_row, n_col)
