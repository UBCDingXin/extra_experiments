...
if not os.path.isfile(Filename_CVAE):
        ...
        # Start training
        net_encoder, net_decoder, train_cvae_loss = train_cvae(images_train, labels_train, net_encoder, net_decoder, save_images_folder=save_images_in_train_folder, save_models_folder = save_models_folder)
...
if args.comp_FID:
    ...
    print("\n Start sampling {} fake images per label from CVAE >>>".format(args.nfake_per_label))

    if args.eval_mode == 1: #Mode 1: eval on unique labels used for CVAE training
...
#######################################################################################
'''               Visualize fake images of the trained CVAE                          '''
#######################################################################################
if args.visualize_fake_images:
    ...
    ### output fake images from a trained CVAE
