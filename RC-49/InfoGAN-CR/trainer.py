...
alpha_cr = args.alpha_cr
cr_gap_tran_niter = args.cr_gap_tran_niter
lr_g = args.lr_g
lr_d = args.lr_d
...
## implementing the contrastive latent sampling in Appendix D of InfoGAN-CR
def sample_latent_code(batch_size, dim_c, cr_gap):
    c_loc_cr = np.random.choice(np.arange(0,dim_c), size=batch_size) #location of selected fixed latent codes
    c_loc_cr = torch.from_numpy(c_loc_cr).type(torch.long).cuda()
    c_cr_1 = [] #store latent codes for CR
    c_cr_2 = []
    for tmp_i in range(batch_size):
        c_cr_1_i = np.zeros(dim_c)
        c_cr_2_i = np.zeros(dim_c)
        for tmp_j in range(dim_c):
            if tmp_j == c_loc_cr[tmp_i].item():
                c_cr_1_i[tmp_j] = np.random.uniform(low=-1,high=1,size=1)
                c_cr_2_i[tmp_j] = np.random.uniform(low=-1,high=1,size=1)
            else:
                c_val_1 = np.random.uniform(low=-1+cr_gap/2,high=1-cr_gap/2,size=1)
                c_val_2 = np.random.uniform(low=-1+cr_gap/2,high=1-cr_gap/2,size=1)
                if c_val_1>c_val_2:
                    c_val_1 += cr_gap/2
                    c_val_2 -= cr_gap/2
                else:
                    c_val_1 -= cr_gap/2
                    c_val_2 += cr_gap/2
                c_cr_1_i[tmp_j] = c_val_1
                c_cr_2_i[tmp_j] = c_val_2
        c_cr_1.append(c_cr_1_i.reshape(1,dim_c))
        c_cr_2.append(c_cr_2_i.reshape(1,dim_c))
    c_cr_1 = np.concatenate(c_cr_1, axis=0)
    c_cr_2 = np.concatenate(c_cr_2, axis=0)
    c_cr_1 = torch.from_numpy(c_cr_1).type(torch.float).cuda()
    c_cr_2 = torch.from_numpy(c_cr_2).type(torch.float).cuda()

    return c_cr_1, c_cr_2, c_loc_cr



def train_infogan(images, cr_gap, netG, netD, netDH, netQH, netCR, save_images_folder, save_models_folder):
    ...
    netCR = netCR.cuda()
    netCR.apply(weights_init_normal)

    ## loss functions
    criterionD = nn.BCELoss() #for discriminator
    criterionQ = NormalNLLLoss() # Loss for continuous latent code.
    criterionCR = nn.CrossEntropyLoss() #cross entropy loss for contrastive regularizer
    ...
    optimizerCR = torch.optim.Adam(netCR.parameters(), lr=lr_d, betas=(0.5, 0.999))
    ...
    if save_models_folder is not None and resume_niters>0:
        save_file = save_models_folder + "/InfoGAN-CR_checkpoint_intrain/InfoGAN-CR_checkpoint_niters_{}.pth".format(resume_niters)
        ...
        netCR.load_state_dict(checkpoint['netCR_state_dict'])
        optimizerG.load_state_dict(checkpoint['optimizerG_state_dict'])
        optimizerD.load_state_dict(checkpoint['optimizerD_state_dict'])
        optimizerCR.load_state_dict(checkpoint['optimizerCR_state_dict'])
    ...
    for niter in range(resume_niters, niters):
        ...
        # for contrastive regularizer
        if niter>=cr_gap_tran_niter:
            cr_gap = 0
        
        ## next, implementing the contrastive latent sampling in Appendix D of InfoGAN-CR
        c_cr_1, c_cr_2, c_loc_cr = sample_latent_code(batch_size, dim_c, cr_gap)


        ## generate fake images for CR
        z = torch.randn(batch_size, dim_z, dtype=torch.float).cuda()
        batch_fake_images_cr_1 = netG(z, c_cr_1)
        z = torch.randn(batch_size, dim_z, dtype=torch.float).cuda()
        batch_fake_images_cr_2 = netG(z, c_cr_2)

        ## CR discriminator output
        out_CR = netCR(batch_fake_images_cr_1, batch_fake_images_cr_2)
        cr_loss = criterionCR(out_CR, c_loc_cr)

        #generator try to let disc believe gen_imgs are real
        g_loss = criterionD(DH_out, GAN_real) + lambda_info * criterionQ(c, q_mu, q_var) - alpha_cr * cr_loss
        ...
        Update the H(x) in the contrastive regularizer
        ...
        if (niter+1)%20 == 0:
            print ("InfoGAN-CR: [Iter %d/%d] [D loss: %.4f] [G loss: %.4f] [D prob real:%.4f] [D prob fake:%.4f] [Time: %.4f]" % (niter+1, niters, d_loss.item(), g_loss.item(), prob_real.mean().item(),prob_fake.mean().item(), timeit.default_timer()-start_time))
        ...
        if save_models_folder is not None and ((niter+1) % save_niters_freq == 0 or (niter+1) == niters):
            ...
            torch.save({
                    ...
                    'netCR_state_dict': netCR.state_dict(),
                    'optimizerG_state_dict': optimizerG.state_dict(),
                    'optimizerD_state_dict': optimizerD.state_dict(),
                    'optimizerCR_state_dict': optimizerCR.state_dict(),
    ...
    return netG, netD, netDH, netQH, netCR
